...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function showResults(results) {
        ...
        // Update stats
        document.getElementById('songs-count').textContent = results.songs_count || 0;

        const stats = results.stats || {};
        document.getElementById('total-words').textContent = (stats.total_words || 0).toLocaleString();
        document.getElementById('unique-words').textContent = (stats.unique_words || 0).toLocaleString();
            ...
            const html = `
                <div class="topic-item">
                    <div class="topic-header">
                        <span class="topic-name">${topic.name || `Topic ${index + 1}`}</span>
                        <span class="topic-weight">${((topic.weight || 0) * 100).toFixed(1)}%</span>
                    </div>
    ...
    function renderSentiment(sentiment) {
        ...
        overallEl.textContent = overall >= 0 ? `+${overall.toFixed(2)}` : overall.toFixed(2);
        ...
        items.forEach(item => {
            ...
            const html = `
                <div class="bar-item">
                    ...
                    <span class="bar-value">${score >= 0 ? '+' : ''}${score.toFixed(2)}</span>
                </div>
            `;
            container.innerHTML += html;
        });
